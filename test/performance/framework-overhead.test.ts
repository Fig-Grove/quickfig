...
import { FigmaConstraintDetector } from '../../dist/index.cjs';
...
// Create framework constraint detector instance
...
test('Constraint detection API validation overhead', async (t) => {
  ...
  for (const api of testApis) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const isAvailable = typeof globalThis[api as keyof typeof globalThis] !== 'undefined';
      const end = performance.now();
      withoutConstraintsTimes.push(end - start);
    }
  ...
  t.true(analysis.summary.overheadAcceptabilityRate >= 40, 'At least 40% of API operations should have acceptable overhead');
});
...
test('Constraint detection under realistic load', async (t) => {
  ...
  t.true(totalTime < 1000, 'Total time for 1k operations should be under 1 second');
