...
interface ConstraintViolation {
  ...
  type: 'memory' | 'ui-blocking' | 'api-compatibility' | 'string-size' | 'execution-time';
  ...
  try {
    ...
    switch (options.command) {
      case 'validate':
        ...
        await executeWithWatch(options, () => validateFile(options.file!, options));
        break;
      ...
      case 'benchmark':
        ...
        await executeWithWatch(options, () => benchmarkFile(options.file!, options));
        break;
      ...
      case 'constraints':
        ...
        await executeWithWatch(options, () => checkConstraints(options.file!, options));
        break;
      ...
      case 'quality-gates':
        ...
        await executeWithWatch(options, () => runQualityGates(options.file!, options));
        break;
...
async function executeWithWatch(options: CLIOptions, command: () => Promise<void>) {
...
async function watchFile(filePath: string, command: () => Promise<void>, options: CLIOptions) {
  ...
  try {
    const watcher = watch(fullPath, { signal: watchAbortController.signal }, (eventType) => {
      if (eventType === 'change') {
        debounceFileChange(async () => {
          const timestamp = new Date().toLocaleTimeString();
          console.log(`🔄 [${timestamp}] File changed, re-running ${options.command}...`);
          console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
          try {
            await command();
            console.log(`✅ [${timestamp}] ${options.command} completed successfully`);
          } catch (error) {
            console.error(`❌ [${timestamp}] ${options.command} failed:`, error);
            if (options.verbose) {
              console.error(error);
            }
          }
...
function displayValidationResults(result: ValidationResult) {
  ...
  console.log(`⚡ UI blocking constraints: ${formatStatus(result.uiBlockingCompliant)}`);
  console.log(`🔗 API compatibility: ${formatStatus(result.apiCompatible)}`);
  console.log();
  console.log(`🎯 Overall: ${result.overallPass ? '✅ APPROVED' : '❌ NEEDS ATTENTION'}`);
...
async function benchmarkFile(filePath: string, options: CLIOptions) {
  try {
    ...
    if (options.output === 'json') {
      ...
      const performanceRating = executionTime < 16 ? 'Excellent' : 
                               executionTime < 100 ? 'Good' : 'Needs Optimization';
...
async function checkConstraints(filePath: string, options: CLIOptions) {
  try {
    ...
    const checkMemory = options.memory || (!options.memory && !options.uiBlocking);
    const checkUIBlocking = options.uiBlocking || (!options.memory && !options.uiBlocking);
    ...
    const uiViolations = result.violations.filter(v => v.type === 'execution');
    ...
    if (options.output === 'json') {
      ...
      if (checkMemory) {
        console.log(`💾 Memory Constraints: ${memoryPass ? '✅ PASS' : '❌ FAIL'}`);
      }
      if (checkUIBlocking) {
        console.log(`⚡ UI Blocking Constraints: ${uiPass ? '✅ PASS' : '❌ FAIL'}`);
      }
      
      console.log(`🎯 Overall Constraint Check: ${overallPass ? '✅ PASSED' : '❌ FAILED'}`);
...
async function runQualityGates(filePath: string, options: CLIOptions) {
  try {
    ...
    if (options.output === 'json') {
      ...
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log(`🎯 Overall Status: ${gateResults.cicdStatus === 'approved' ? '✅ PASSED' : '❌ FAILED'}`);
      console.log(`📊 Quality Score: ${gateResults.overallScore}/100`);
      console.log(`📋 Tests: ${gateResults.passedTests}/${gateResults.totalTests} passed`);
      console.log(`⚠️ Failures: Critical=${gateResults.criticalFailures}, Major=${gateResults.majorFailures}, Minor=${gateResults.minorFailures}`);
