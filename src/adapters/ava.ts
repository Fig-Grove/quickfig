...
export function avaAdapter(test: TestFn) {
  return {
    quickjs: (name: string, testFn: (t: any, ctx: AVAQuickJSTestContext) => Promise<void>) => {
      test(name, async (t) => {
        const ctx: AVAQuickJSTestContext = {
          ...
          createHarness: async (options?: QuickJSTestOptions) => {
            return await createIsolatedFigmaTestEnvironment(name, options || {});
          },
          
          assertMemoryCompliant: (result: ConstraintValidationResult) => {
            t.true(result.memoryCompliant, `Memory constraints violated: ${result.violations.filter(v => v.type === 'memory').map(v => v.message).join(', ')}`);
          },
          
          assertUIBlockingCompliant: (result: ConstraintValidationResult) => {
            t.true(result.uiBlockingCompliant, `UI blocking constraints violated: ${result.violations.filter(v => v.type === 'execution').map(v => v.message).join(', ')}`);
          }
        };
