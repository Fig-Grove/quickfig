...
function createSandboxedFunctionWrapper(originalRunSandboxed: any) {
  return async function runSandboxed(code: string | (() => any), options?: SandboxOptions): Promise<any> {
    const config = { 
    ...
    if (typeof code === 'string') {
      ...
      return originalRunSandboxed(async ({ evalCode }: { evalCode: (code: string) => any }) => {
      ...
      const standardMatch = functionBody.match(/^(?:async\s+)?(?:function\s*)?\([^)]*\)\s*(?:=>\s*)?{\s*([\s\S]*?)\s*}$/);
      if (standardMatch) {
        codeBody = standardMatch[1];
      } else {
        // Strategy 2: Arrow function with implicit return (no braces)
        const arrowMatch = functionBody.match(/^(?:async\s+)?\([^)]*\)\s*=>\s*(.+)$/);
        if (arrowMatch) {
          ...
          const singleArrowMatch = functionBody.match(/^(?:async\s+)?[^(=>\s]+\s*=>\s*(.+)$/);
          if (singleArrowMatch) {
            ...
            if (firstBrace !== -1 && lastBrace !== -1 && firstBrace < lastBrace) {
              ...
              throw new Error(`Unable to parse function code for sandboxed execution. Function body: ${functionBody.substring(0, 100)}...`);
            }
          }
        }
      }
      ...
      if (hasReturn) {
        ...
        if (trimmedBody.startsWith('{') && trimmedBody.endsWith('}') && !trimmedBody.includes(';')) {
          ...
          if (lastLine && !lastLine.startsWith('return') && !lastLine.includes(';') && !lastLine.includes('=')) {
            // Last line looks like an expression, wrap it with return
            const otherLines = lines.slice(0, -1).join('\n');
            wrappedCode = otherLines + (otherLines ? '\n' : '') + `return ${lastLine}`;
          } else {
            // Use the code as-is
            wrappedCode = codeBody;
          }
