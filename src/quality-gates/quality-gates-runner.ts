...
export class QualityGateRunner {
  ...
  async run(): Promise<QualityGateResults> {
    ...
    console.log(
      `🎯 Thresholds: Critical≤${this.config.thresholds.criticalFailuresMax}, Major≤${this.config.thresholds.majorFailuresMax}, Score≥${this.config.thresholds.overallScoreMin}`,
    );
    ...
    try {
      ...
      this.results.duration =
        Date.now() - new Date(this.results.startTime).getTime();
    }
  }
  ...
  private async runQualityGateTests(): Promise<void> {
    ...
    for (const testPattern of this.config.testPatterns) {
      ...
      try {
        ...
        console.log(
          `✅ ${testPattern}: ${testResult.passedTests}/${testResult.totalTests} passed`,
        );
      } catch (error) {
  ...
  private parseAvaOutput(avaOutput: string, testFile: string): TestResult {
    ...
    for (const line of lines) {
      ...
      if (
        line.includes("constraint violation") ||
    ...
    const timeMatch = avaOutput.match(
      /(\d+) tests? passed.*?\((\d+(?:\.\d+)?)s\)/,
    );
  ...
  private evaluateResults(): void {
    ...
    for (const result of this.results.testResults) {
      if (result.failedTests > 0) {
        ...
        if (
          result.testFile.includes("performance") ||
          result.testFile.includes("constraint")
        ) {
    ...
    if (
      this.results.criticalFailures > this.config.thresholds.criticalFailuresMax
    ) {
      ...
      this.results.majorFailures > this.config.thresholds.majorFailuresMax
    ) {
      ...
      this.results.overallScore < this.config.thresholds.overallScoreMin
    ) {
      this.results.cicdStatus = "warning";
      ...
      const jsonReport = {
        ...
        environment: {
          ...
          branch:
            ...
            "unknown",
          commit:
            process.env.GITHUB_SHA || process.env.CI_COMMIT_SHA || "unknown",
        },
      };
  ...
  private generateJUnitReport(): string {
    const testSuites = this.results.testResults
      .map((result) => {
        ...
        for (let i = 0; i < result.passedTests; i++) {
          testCases.push(
            `    <testcase name="${result.testFile}-${i + 1}" classname="QuickJSQualityGates" time="${(result.executionTime / 1000 / result.totalTests).toFixed(3)}"/>`,
          );
        }
...
`;
  ...
  private displaySummary(): void {
    ...
    console.log(
      `✅ Passed Tests: ${this.results.passedTests}/${this.results.totalTests}`,
    );
    console.log(`❌ Failed Tests: ${this.results.failedTests}`);
    console.log(
      `🚨 Critical Failures: ${this.results.criticalFailures} (constraints/performance)`,
    );
    console.log(
      `⚠️  Major Failures: ${this.results.majorFailures} (integration)`,
    );
    console.log(
      `ℹ️  Minor Failures: ${this.results.minorFailures} (unit tests)`,
    );
