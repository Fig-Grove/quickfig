...
export {
  BenchmarkRegressionTracker,
  extractBenchmarkMetrics,
} from "./benchmarks/regression-tracker.js";
...
export async function validateFigmaPluginConstraints(
  ...
  const { createIsolatedFigmaTestEnvironment } = await import(
    "./harness/isolated-quickjs-harness.js"
  );
...
function estimateCodeMemoryUsage(code: string): number {
  ...
  const stringLiterals = (code.match(/["'`][^"'`]*["'`]/g) || []).join(
    "",
  ).length;
  ...
  return (
    lines * 100 + stringLiterals * 2 + arrayLiterals * 10 + objectLiterals * 20
  );
}
