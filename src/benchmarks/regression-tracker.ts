...
export class BenchmarkRegressionTracker {
  ...
  constructor(config: Partial<RegressionDetectionConfig> = {}) {
    this.config = {
      performanceRegressionThreshold:
        config.performanceRegressionThreshold ?? 15, // 15% slower
   ...
   */
  analyzeRegressions(
    currentMetrics: Array<Omit<BenchmarkMetric, "timestamp">>,
  ): RegressionReport {
    const history = this.loadMetricsHistory();
    ...
    for (const currentMetric of currentMetrics) {
      ...
      } else if (regressionAnalysis.percentageChange < -5) {
        // 5% improvement
    ...
    const improvementBonus = Math.min(
      report.summary.improvementsDetected * 5,
      20,
    );
    report.summary.overallScore = Math.max(
      0,
      100 - regressionPenalty + improvementBonus,
    );
  ...
  getTrends(
    ...
    const metricHistory = history
      .filter(
        (m) => m.name === metricName && new Date(m.timestamp) > cutoffTime,
      )
      .sort(
        (a, b) =>
  ...
  generateDashboard(): {
    ...
    const recentMetrics = history.filter(
      (m) => new Date(m.timestamp) > last7Days,
    );
    ...
    const healthScore =
      recentReports.length > 0
        ? recentReports.reduce(
            (sum, report) => sum + report.summary.overallScore,
            0,
    ...
    const recentRegressions = recentReports.reduce(
      (sum, report) => sum + report.summary.regressionsDetected,
      0,
    );
    const recentImprovements = recentReports.reduce(
      (sum, report) => sum + report.summary.improvementsDetected,
      0,
    );
  ...
  private detectRegression(
    ...
    const percentageChange =
      ((currentMetric.value - baseline.average) / baseline.average) * 100;
  ...
  private generateRecommendation(
    ...
    switch (metric.category) {
      case "performance":
        if (regressionAnalysis.severity === "critical") {
          recommendations.push(
            "URGENT: Critical performance regression detected.",
          );
          recommendations.push("Review recent changes and consider rollback.");
          recommendations.push(
            "Profile the affected code path for optimization opportunities.",
          );
        } else {
          recommendations.push("Performance degradation detected.");
          recommendations.push(
            "Consider profiling and optimizing the affected functionality.",
          );
        }
        break;
      ...
      case "memory":
        ...
        recommendations.push(
          "Consider implementing memory pooling or cleanup strategies.",
        );
        break;
      ...
      case "constraint":
        ...
        recommendations.push(
          "Consider tightening constraint detection thresholds.",
        );
        break;
